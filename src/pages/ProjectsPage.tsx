      ...
      >
        ...
        >
          ...
          <p className="text-muted-foreground mt-3 text-sm sm:text-base max-w-2xl">
            A selection of production applications, open-source tools, and
            engineering experiments shipped across fintech, healthcare, and
            developer tooling.
          </p>
        </motion.div>
        ...
        >
          {projects.map((project) => (
            <motion.a
              ...
              className={`group grid grid-cols-1 sm:grid-cols-[120px_180px_1fr] md:grid-cols-[140px_200px_1fr] gap-1 sm:gap-4 py-5 sm:py-6 border-b border-border hover:bg-secondary/30 -mx-4 px-4 sm:-mx-6 sm:px-6 transition-colors duration-200 ${project?.link ? "cursor-pointer" : "cursor-default opacity-70"}`}
            >
              {/* Date / Tag */}
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from(new Set(projects.flatMap((p) => p.techStack))).map(
              (tech) => (
                <span
                  key={tech}
